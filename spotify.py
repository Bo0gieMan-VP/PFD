...
def is_an_album(album):
    albums = os.listdir('albums_genius')
    if album in albums:
        songs = os.listdir('albums_genius/' + album)
        return len(songs)
    return 0
...
def get_album_by_song(songname):
    for album in os.listdir('albums_genius'):
        songs = [i.replace(".txt", "").lower() for i in os.listdir('albums_genius/'+ album)]
        if songname.lower() in songs:
            return album
    return -1

def get_song_names(album):
    path = "albums_genius/" + album
...
def create_folder(album):
    if not os.path.exists("albums_spotify/" + album['name']):
        os.mkdir("albums_spotify/" + album['name'])
    tracks = [clean_name(i['name']) for i in get_tracks(album)]
    for track in tracks:
        song_path = "albums_spotify/" + album['name'] + "/" + track + ".txt"
...
def get_lyrics(songname):
    lyrics = None
    list_of_folders = list(os.listdir("albums_genius"))
    for folder in list_of_folders:
        path = "albums_genius/" + folder + "/" + songname.title() + ".txt"
        if os.path.exists(path):
...
def get_songs_by_lyrics(lyrics):
    songs_list = []
    folders = os.listdir("albums_genius")
    for folder in folders:
        songs = os.listdir("albums_genius/" + folder)
        for song in songs:
            path = "albums_genius/"+folder+"/"+song
            with open(path, 'r') as f:
